...
#Else, use a .csv file generated from workflow_velstructure
...
###############################################################################
   def output_1D_bm(self):
###############################################################################
       '''
       Returns .sph file for the Mantle_Structure class. This can be used in the
       inparam_hetero file of AxiSEM.
       '''
#Get horizontal average of vp, vs, density
       avg_vp = np.flipud(np.average(self.vp_2D,1))
       avg_vs = np.flipud(np.average(self.vs_2D,1))
       avg_density = np.flipud(np.average(self.rho_2D,1))
       radius = np.flipud(self.radius)
       file = open(self.name+'_1D.bm','w+')
       file.write('ANELASTIC           T \n')
       file.write('ANISOTROPIC         F \n')
       file.write('UNITS               F \n')
       file.write('COLUMNS   radius   rho   vpv   vsv \n')
       for ii in range(0,len(self.radius)):
               file.write(str(radius[ii]*1000.)+' '+ \
                          str(np.round(avg_density[ii],decimals=2))+' '+ \
                          str(avg_vp[ii]*1000.)+' '+ \
                          str(avg_vs[ii]*1000.)+'\n')
       file.close()
...
###############################################################################
       ...
       for ii in range(0,len(self.reshape_radius)):
           file.write(str(np.around(self.reshape_radius[ii][0],decimals=3))+' '+ \
                   ...
                   str(1000.*np.around(self.reshape_vp[ii][0],decimals=3))+' '+ \
                   str(1000.*np.around(self.reshape_vs[ii][0],decimals=3))+' '+ \
